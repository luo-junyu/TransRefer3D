...
class ListeningDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        res['word_mask'] = (tokens != 0).astype(np.int32)
        res['sent_len'] = np.sum(res['word_mask'].astype(np.int32))
        ...
        res['object_mask'] = np.zeros((self.max_context_size, ), dtype=int)
        res['object_mask'][:len(samples)] = 1
