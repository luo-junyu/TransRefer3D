...
class LSTMEnc(nn.Module):
    def __init__(self, dim, bidirectional=False):
        super().__init__()
        self.dim = dim
        self.rnn = nn.LSTM(input_size=dim, hidden_size=dim,
                           bidirectional=bidirectional, batch_first=True)
        if self.out_dim() != self.dim:
            self.mapping = nn.Linear(self.out_dim(), self.dim)
    ...
    def __call__(self, x, len_of_sequence):  # x: B x nowrd x dim
        seq_len = x.shape[1]
        x_packed = pack_padded_sequence(x, len_of_sequence, enforce_sorted=False, batch_first=True)

        self.rnn.flatten_parameters()
        rnn_out, _ = self.rnn(x_packed)

        rnn_out, dummy = pad_packed_sequence(rnn_out, batch_first=True, total_length=seq_len)  # B x nword x *

        if self.out_dim() != self.dim:
            rnn_out = self.mapping(rnn_out)
        return rnn_out
